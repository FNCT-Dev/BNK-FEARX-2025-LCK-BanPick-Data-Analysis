...
##### Step 2.1.1.1. BFX 시점 블루 진영 전체 모스트 픽

```{r}
bfx_comp_only_blue_pickpref <- bfx_comp_only_blue %>%
  pivot_longer(cols = pick1:pick5, names_to = "pick_order", values_to = "champion") %>%
  group_by(champion) %>%
  summarise(games = n(), .groups = "drop") %>%
  arrange(pick_order, desc(games)) %>%
  write_csv("data/Complicated/20250907 BNK FEARX Complicated Only Blueside Pickpref.csv")
```

##### Step 2.1.1.2. BFX 시점 블루 진영 1픽 모스트 픽
...
##### Step 2.1.3.2. BFX전 종합 블루 진영 1픽 모스트 픽
